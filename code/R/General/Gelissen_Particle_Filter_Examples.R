library(mvtnorm)
#source("/Users/avivb/Google Drive/Research/StanfordPostdoc/mehcoeg/code/R/General/PF.R")
# source("/Users/avivb/Google Drive/Research/StanfordPostdoc/mehcoeg/code/R/General/PF_mod.R")
...
for (i in 2:(length(t)+1)) {
    Xk[i] <- Xk[i-1] +1 #+ rnorm(1)*Wk
    Yk[i-1] = Xk[i] #+ rnorm(1)*Vk
}
...
# 
# #95% ci for y
# seFY <- sqrt(unlist(lapply(pf4$C, function(x) diag(x)[1])))
# ciFY <- pf4$m[,1] + qnorm(.05/2)*seFY%o%c(1, -1)
# #95% ci for vy
# seFVY <- sqrt(unlist(lapply(pf4$C, function(x) diag(x)[2])))
# ciFVY <- pf4$m[,2] + qnorm(.05/2)*seFVY%o%c(1, -1)
# #95% ci for bc
# seFBC <- sqrt(unlist(lapply(pf4$C, function(x) diag(x)[3])))
