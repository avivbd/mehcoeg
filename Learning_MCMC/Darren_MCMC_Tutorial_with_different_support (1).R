...
propdist1 <- function(x, mu = 0 , eps = 1){
  #dense = dnorm(x, mu, eps) #draw from some dist
  #dense = dexp(x)  #draw from some dist
  dense = runif(1, -eps, eps) #draw from some dist
}

randraw1 <- function(x){
  #innov = rnorm(1) #should be same distribution as proposal dist above
  #innov = rexp(1) #should be same distribution as proposal dist above
  innov = runif(1) #should be same distribution as proposal dist above
...
mcmc_out <- metrop1(n = 10000, x0 = 0, eps= 1, unknfun = unkownfun1, propdist = propdist1)
plot_mcmc(mcmc_out)
